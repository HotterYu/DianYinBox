...
public class SettingActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (!mHasPermission && WifiSupport.isOpenWifi(SettingActivity.this)) {  //未获取权限，申请权限
            requestPermission();
        }else if(mHasPermission && WifiSupport.isOpenWifi(SettingActivity.this)){  //已经获取权限
            //initRecycler();
        }else{
            Toast.makeText(SettingActivity.this,"WIFI处于关闭状态",Toast.LENGTH_SHORT).show();
        }
        initRecycler();
    ...
    private void initRecycler() {
        ...
        if(WifiSupport.isOpenWifi(SettingActivity.this) && mHasPermission){
            sortScaResult();
        }else{
            Toast.makeText(SettingActivity.this,"WIFI处于关闭状态或权限获取失败22222",Toast.LENGTH_SHORT).show();
        }
            ...
            public void onItemClick(View view, int postion, Object o)
            {
                ...
                if(wifiBean.getState().equals(AppContants.WIFI_STATE_UNCONNECT) || wifiBean.getState().equals(AppContants.WIFI_STATE_CONNECT)){
                    ...
                    if(WifiSupport.getWifiCipher(capabilities) == WifiSupport.WifiCipherType.WIFICIPHER_NOPASS){//无需密码
                        WifiConfiguration tempConfig  = WifiSupport.isExsits(wifiBean.getWifiName(),SettingActivity.this);
                        if(tempConfig == null)
                        {
                            ...
                            WifiSupport.addNetWork(exsits, SettingActivity.this);
                        }
                        else
                            {

                            WifiSupport.addNetWork(tempConfig, SettingActivity.this);
                        }
    ...
    private void noConfigurationWifi(int position) {//之前没配置过该网络， 弹出输入密码界面
        final WifiLinkDialog linkDialog = new WifiLinkDialog(this,R.style.dialog_download,realWifiList.get(position).getWifiName(), realWifiList.get(position).getCapabilities());
        if(!linkDialog.isShowing())
        {
            ...
            linkDialog.setOnDismissListener(new DialogInterface.OnDismissListener() {
                ...
                {
                    if(linkDialog.isConfirmClick())
                    {
                        String wifiName = linkDialog.getWifiName();
                        String wifiPwd = linkDialog.getWifiPwd();

                        Intent intent = new Intent();
                        Bundle b = new Bundle();
                        b.putString("WIFI_NAME",wifiName);
                        b.putString("WIFI_PWD",wifiPwd);
                        intent.putExtras(b);
        ...
        public void onReceive(Context context, Intent intent) {
            if(WifiManager.WIFI_STATE_CHANGED_ACTION.equals(intent.getAction())){
                ...
                if(NetworkInfo.State.DISCONNECTED == info.getState()){//wifi没连接上
                    ...
                    //连接成功 跳转界面 传递ip地址
                    Toast.makeText(SettingActivity.this,"wifi连接上了",Toast.LENGTH_SHORT).show();
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == PERMISSION_REQUEST_CODE) {
            ...
            if (hasAllPermission) {
                mHasPermission = true;
                if(WifiSupport.isOpenWifi(SettingActivity.this) && mHasPermission){  //如果wifi开关是开 并且 已经获取权限
                    initRecycler();
                }else{
                    Toast.makeText(SettingActivity.this,"WIFI处于关闭状态或权限获取失败1111",Toast.LENGTH_SHORT).show();
                }

            } else {  //用户不同意权限
                mHasPermission = false;
                Toast.makeText(SettingActivity.this,"获取权限失败",Toast.LENGTH_SHORT).show();
            }
        }
    }
