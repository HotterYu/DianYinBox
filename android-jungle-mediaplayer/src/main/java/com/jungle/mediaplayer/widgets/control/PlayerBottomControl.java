...
public class PlayerBottomControl extends FrameLayout {
    ...
    private TextView mCurPlayName;
    private View viewVolume = null;
    private ImageView ivRotation = null;
    ...
    public void create(int resId) {
        ...
        mCurPlayName = (TextView) findViewById(R.id.player_curr_media_name);
        mSeekBar = (SeekBar) findViewById(R.id.player_progress);
        viewVolume = findViewById(R.id.player_volume_btn);
        ...
        ivRotation.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                mOperationHelper.switchRoation();
            }
        });
    ...
    public void setmCurPlayName(JungleMediaPlayer.PlayVideoInfo mSavedVideoInfo)
    {
        String mediaName = mSavedVideoInfo.mVideoInfo.getMediaName();
        String mediaType = mSavedVideoInfo.mVideoInfo.getMediaType();
        String playType = mSavedVideoInfo.mVideoInfo.getPlayType();

        if(mediaType.equals("0"))//计划歌曲
        {
            mCurPlayName.setText(mediaName);
        }
        else if(mediaType.equals("1"))//广告
        {
            mCurPlayName.setText("广告->"+mediaName);
        }
        else if(mediaType.equals("2"))//插播
        {
            mCurPlayName.setText("插播->"+mediaName);
        }

        if(playType.equals("1"))
        {
            mCurPlayName.setText("在线  " + mCurPlayName.getText());
        }
        else if(playType.equals("0"))
            mCurPlayName.setText("本地  " + mCurPlayName.getText());
    }
    ...
    public void onStartPlayProcess()
    {
    ...
    public void onResumedPrcess()
    {
        mPlayPauseBtn.setImageResource(R.drawable.player_icon_media_pause);
    }

    public void onPausedProcess(){
        mPlayPauseBtn.setImageResource(R.drawable.player_icon_media_play);
    }
    public void onStoppedProcess(){
        mPlayPauseBtn.setImageResource(R.drawable.player_icon_media_play);
    }

    public void onPlayCompleteProcess()
    {
        clearUpdateProgress();
    }
